...
MU_TEST(test_file_read_write) {
        ...
        mu_assert(sys_file_open_read(rpath, &rid, &size) == SYS_OK, "Failed to open a file.");
        char* rdata = calloc(2, sizeof(char));
        mu_assert(sys_file_read(rid, rdata, 1) == SYS_OK, "Failed to read a file.");
        rdata[1] = '\0';
        mu_assert(strcmp((const char*)rdata, "R") == 0, "Wrong data after file read.");
        mu_assert(sys_file_seek(rid, 0) == SYS_OK, "Failed to seek on file.");
        MString wpath = mstr_from_cstr(test_write_path);
        FileHandleID wid;
        mu_assert(sys_file_open_write(wpath, &wid) == SYS_OK, "Failed to open file for writing.");
        void* data = calloc(size, sizeof(u8));
        mu_assert(sys_file_read(rid, data, size) == SYS_OK, "Failed to read a file.");
        mu_assert(sys_file_write(wid, data, size) == SYS_OK, "Failed to write to a file.");
